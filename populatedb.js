...
async function createAvaliations() {
    console.log("Adding avaliations");
    await Promise.all([
...
async function avaliationCreate(index, user_name, rating, comment) {
    const avaliationdetail = {
        user_name,
        rating,
    };
    if (comment != false) avaliationdetail.comment = comment;

    const avaliation = new Avaliation(avaliationdetail);
    await avaliation.save();
    avaliations[index] = avaliation;
    console.log(`Added avaliation by: ${user_name}`);
}
